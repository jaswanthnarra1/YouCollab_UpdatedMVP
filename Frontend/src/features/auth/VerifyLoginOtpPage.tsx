...
/**
 * Completes sign-in when Clerk's environment requires a second factor
 * (email_code) on top of the password — signIn.create() returns
 * status "needs_second_factor" in that case instead of "complete", and
 * AuthPage redirects here with prepareSecondFactor already called.
 */
      ...
      <main className="mx-auto flex w-full max-w-[440px] flex-col px-4 pt-16 pb-20">
        <div className="border border-border rounded-md p-8 space-y-6 bg-background">
          ...
          <form className="space-y-4" onSubmit={submitVerify}>
            ...
            >
              {loading ? <Loader2 className="h-4 w-4 animate-spin" /> : (
                <>Verify & Sign In <ArrowRight className="h-3.5 w-3.5" /></>
              )}
            </Button>
          </form>
          ...
          <div className="pt-4 border-t border-border flex flex-col gap-2 items-center text-xs text-muted-foreground">
            ...
            <Link to="/login" className="hover:underline mt-1 text-[11px]">
              Back to login
            </Link>
          </div>
        </div>
