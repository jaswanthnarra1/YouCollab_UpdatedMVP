...
-- "Nearby" dashboard widget: closest N counterpart profiles ranked by
-- distance from the requester's own PIN code. Not paginated (top-N list,
-- not a feed) — mirrors list_gigs_in_radius's haversine_km usage but reads
-- influencers/brands directly instead of gigs.
CREATE OR REPLACE FUNCTION list_nearest_influencers(
  p_lat DOUBLE PRECISION,
  p_lng DOUBLE PRECISION,
  p_limit INTEGER
)
RETURNS TABLE (
  ...
  SELECT i.id, i.name, i."instagramHandle", i.niche, i."profileImageUrl", i."followerCount",
    ROUND((haversine_km(i.latitude, i.longitude, p_lat, p_lng) * 2)::numeric)::double precision / 2 AS distance_km
  FROM influencers i
  WHERE i.latitude IS NOT NULL AND i.longitude IS NOT NULL
  ORDER BY haversine_km(i.latitude, i.longitude, p_lat, p_lng) ASC
  LIMIT p_limit;
$$ LANGUAGE sql STABLE;

CREATE OR REPLACE FUNCTION list_nearest_brands(
  p_lat DOUBLE PRECISION,
  p_lng DOUBLE PRECISION,
  p_limit INTEGER
)
RETURNS TABLE (
  ...
  SELECT b.id, b."businessName", b.category, b."logoUrl", b.location,
    ROUND((haversine_km(b.latitude, b.longitude, p_lat, p_lng) * 2)::numeric)::double precision / 2 AS distance_km
  FROM brands b
  WHERE b.latitude IS NOT NULL AND b.longitude IS NOT NULL
  ORDER BY haversine_km(b.latitude, b.longitude, p_lat, p_lng) ASC
  LIMIT p_limit;
$$ LANGUAGE sql STABLE;

GRANT EXECUTE ON FUNCTION list_nearest_influencers(DOUBLE PRECISION, DOUBLE PRECISION, INTEGER) TO anon, authenticated;
GRANT EXECUTE ON FUNCTION list_nearest_brands(DOUBLE PRECISION, DOUBLE PRECISION, INTEGER) TO anon, authenticated;
